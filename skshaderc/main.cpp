...
compiler_settings_t check_settings(int32_t argc, const char **argv, bool *exit) {
	...
	// Default language target, spirv
	if (!set_targets) {
		result.shaderc.target_langs[skr_shader_lang_spirv] = true;
	}
...
class Material%s : Material
{
	...
	sksc_shader_buffer_t *buff = &file->meta->buffers[file->meta->global_buffer_id];
	for (uint32_t i=0; i<buff->var_count; i+=1) {
		sksc_shader_var_t *v = &buff->vars[i];
