...
void bloom_create(int32_t width, int32_t height, int32_t mip_count) {
	...
	for (int32_t i = 0; i < mip_count; i++) {
		g_bloom.bloom_chain   [i] = skr_tex_create(skr_tex_fmt_rgba128, skr_tex_flags_readable | skr_tex_flags_compute, linear_clamp, (skr_vec3i_t){mip_width, mip_height, 1}, 1, 1, NULL);
		g_bloom.bloom_upsample[i] = skr_tex_create(skr_tex_fmt_rgba128, skr_tex_flags_readable | skr_tex_flags_compute, linear_clamp, (skr_vec3i_t){mip_width, mip_height, 1}, 1, 1, NULL);
	...
	if (app_read_file("shaders/bloom_downsample.hlsl.sks", &data, &size)) {
		g_bloom.bloom_downsample_shader = skr_shader_create(data, size);
		free(data);
	}
	...
	if (app_read_file("shaders/bloom_upsample.hlsl.sks", &data, &size)) {
		g_bloom.bloom_upsample_shader = skr_shader_create(data, size);
		free(data);
	}
	...
	if (app_read_file("shaders/bloom_composite.hlsl.sks", &data, &size)) {
		g_bloom.bloom_composite_shader = skr_shader_create(data, size);
		free(data);
	}
	printf("  Composite shader loaded:  %d\n", skr_shader_is_valid(&g_bloom.bloom_composite_shader ));

	// Create compute instances
	g_bloom.bloom_composite_mat = skr_material_create((skr_material_info_t){
	...
	for (int32_t i = 0; i < mip_count; i++) {
		g_bloom.bloom_downsample_comp[i] = skr_compute_create(&g_bloom.bloom_downsample_shader);
		g_bloom.bloom_upsample_comp  [i] = skr_compute_create(&g_bloom.bloom_upsample_shader  );
	}
	...
	g_bloom.vertex_type = skr_vert_type_create(
		(skr_vert_component_t[]){ 
			{ .format = skr_vertex_fmt_f32, .count = 2, .semantic = skr_semantic_position, .semantic_slot = 0 },
			{ .format = skr_vertex_fmt_f32, .count = 2, .semantic = skr_semantic_texcoord, .semantic_slot = 0 }}, 2);


	g_bloom.fullscreen_quad = skr_mesh_create(&g_bloom.vertex_type, skr_index_fmt_u16, 
	...
	for (int32_t i = 0; i < mip_count; i++) {
		...
		g_bloom.bloom_params_buffers[i] = skr_buffer_create(&params, 1, sizeof(bloom_params_t), skr_buffer_type_constant, skr_use_dynamic);
		mip_width  /= 2;
		mip_height /= 2;
	}

	composite_params_t composite_params = { .bloom_strength = 0.0f };
	g_bloom.composite_params_buffer = skr_buffer_create(&composite_params, 1, sizeof(composite_params_t), skr_buffer_type_constant, skr_use_dynamic);
